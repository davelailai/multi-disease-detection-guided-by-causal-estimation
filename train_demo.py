...
from mmpretrain.engine.runners import DALI_runner,torchvision_runner


def parse_args():
    parser = argparse.ArgumentParser(description='Train a model')
    parser.add_argument('--config', help='train config file path', default='configs/UWF/Unspervised_mae_e100.py')
...
def main():
    ...
    cfg._cfg_dict.train_dataloader.batch_size=1
    cfg._cfg_dict.work_dir='./work_dirs_MICCAI_test/ODIR_7/resnet50_448/Q2L_causal_label_relation_d30_lr5e-3_e100'

    # build the runner from config
    if 'runner_type' not in cfg:
        # build the default runner
        runner = Runner.from_cfg(cfg)
    elif cfg['runner_type']=='DALI_runner':
        runner = DALI_runner.from_cfg(cfg)
    elif cfg['runner_type']=='torchvision_runner':
        runner = torchvision_runner.from_cfg(cfg)
    else:
