    ...
    **Support** is the number of samples:
    ...
    def process(self, data_batch, data_samples: Sequence[dict]):
        ...
        for data_sample in data_samples:
            ...
            # result['gt_score'] = data_sample['gt_label'].clone()
...
class AverageAUC(BaseMetric):
    ...
    def compute_metrics(self, results: List):
        ...
        ap = self.calculate(pred, target, average = self.average, thr=self.thr, topk=self.topk)

        result_metrics = dict()

        if self.average is None:
            result_metrics['AUC_classwise'] = ap.tolist()
        else:
            result_metrics['mAUC'] = ap.item()
    ...
    def calculate(
        ...
        return roc_auc_score(target, pos_inds, average=average)
    
@METRICS.register_module()
class multiclassAverageAUC(BaseMetric):
    default_prefix: Optional[str] = 'multi-class'
    ...
    def compute_metrics(self, results: List):
        ...
        result_metrics = dict()
        result_metrics['mAUC_multi_class'] = ap
    ...
    def calculate(
        ...
        return roc_auc_score(target, pred, multi_class=multi_class,average=average)
...
class ROC_AUC(BaseMetric):
    ...
    def compute_metrics(self, results: List):
        ...
        tpr_list, fpr_list, threshold_list, auc_list = self.calculate(pred, target, average = self.average)

        result_metrics = dict()
        result_metrics['tpr_list']=tpr_list
        result_metrics['fpr_list']=fpr_list
        result_metrics['threshold_list']=threshold_list
        result_metrics['auc_list']=auc_list
        # result_metrics['auc_list_mean']=np.mean(auc_list.copy())
        # if self.average is None:
        #     result_metrics['AUC_classwise'] = ap.tolist()
        # else:
        #     result_metrics['mAUC'] = ap.item()
    ...
    def calculate(
        ...
        auc_list=[]
        for k in range(num_classes):
            cur_fpr, cur_tpr, cur_threshold = roc_curve(target[:, k], pred[:, k])
            auc=roc_auc_score(target[:, k], pred[:, k], average=average)
        ...
        return tpr_list, fpr_list, threshold_list, auc_list
