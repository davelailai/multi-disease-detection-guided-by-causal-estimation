...
class Qeruy2Label_New(MultiLabelClsHead):
    def __init__(self,in_channels, num_classes,position_embedding,img_size, hidden_dim=512, nheads=8, enc_layers=6,
                 ...
                 causal_reg=False,
                 causal_num=False,
                 s=1.0, lambda1=0.02, m=0.1, weight=1e-2,
                 **kwargs):
        """[summary]
        ...
        if self.causal:
            ...
            self.w = weight   
            if self.causal_num:
                self.DagmaMLP= DagmaMLP(dims=[num_classes+causal_num, d2, 1], bias=True,dtype=torch.float)
            else:
                self.DagmaMLP= DagmaMLP(dims=[num_classes, d2, 1], bias=True,dtype=torch.float)
            
            
        if self.causal and self.causal_num:
            self.query_embed = nn.Embedding(num_classes+causal_num, hidden_dim)
        else:
