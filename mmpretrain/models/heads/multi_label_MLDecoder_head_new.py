...
class MLdecoderHead_New(MultiLabelClsHead):
    ...
    def __init__(self, num_classes, num_of_groups=-1, decoder_embedding=768,
                in_channels=2048, zsl=0, causal=False,
                d2=10,
                feature_reg=False, 
                causal_reg=False,
                causal_num=False,
                s=1.0, lambda1=0.02, m=0.1, mu_dec=0.8, weight=1e-2, 
                **kwargs):
        super(MLdecoderHead_New, self).__init__(**kwargs)
        self.num_classes = num_classes
        self.causal_num=causal_num
        self.causal = causal
        ...
        if self.causal and self.causal_num:
            embed_len_decoder=embed_len_decoder+causal_num
        ...
        if self.causal:
            ...
            self.mu=m
            self.w = weight    
            self.DagmaMLP= DagmaMLP(dims=[embed_len_decoder, d2, 1], bias=True,dtype=torch.float)
    ...
    def forward(self, x):
        h=self.head_feature(x)
        B,C,D=h.shape
        if self.causal_reg &self.feature_reg:
            h= h.permute(0,2,1).contiguous().view(-1,C)
            h = self.DagmaMLP(h)
            h = h.reshape(B,D,C).permute(0,2,1).contiguous()
            
        h = h[:,0:self.num_classes,:]
    ...
    def loss(self, feats: Tuple[torch.Tensor], data_samples: List[DataSample],
        ...
        if self.causal:
            if self.feature_reg:
                x=self.head_feature(feats)
                B,C,D=x.shape
                x= x.permute(0,2,1).contiguous().view(-1,C)
            else:
                x = self.decoder.query_embed.weight
                x = x.permute(1,0)
            causal_loss = self.causal_estimation(x)
