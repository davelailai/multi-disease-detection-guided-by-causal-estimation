...
class Qeruy2Label(MultiLabelClsHead):
    def __init__(self,num_channels, num_class,position_embedding,img_size, hidden_dim=512, nheads=8, enc_layers=6,
        ...
        self.causal=causal
        if self.causal:
            self.DagmaMLP= DagmaMLP(dims=[hidden_dim, 10, 1], bias=True,dtype=torch.float)
            self.s = s
    ...
    def causal_estimation(self):
        ...
        X_hat = self.DagmaMLP(self.query_embed.weight)
        score = self.log_mse_loss(X_hat, self.query_embed.weight)
