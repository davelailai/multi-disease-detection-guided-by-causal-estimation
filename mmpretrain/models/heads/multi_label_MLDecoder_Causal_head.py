...
class MLCausaldecoderHead(MultiLabelClsHead):
    ...
    def __init__(self, num_classes, num_of_groups=-1, decoder_embedding=768,
                initial_num_features=2048, zsl=0, s=1.0, lambda1=0.02, m=0.1, weight=1e-3, **kwargs):
        super(MLCausaldecoderHead, self).__init__(**kwargs)
    ...
    def forward(self, x):
        ...
        h = h.transpose(0, 1)
        # X_hat = self.DagmaMLP(h)
        # score = self.log_mse_loss(X_hat, h)
