...
@TRANSFORMS.register_module()
class LoadImageFromPkl(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        img=mmengine.load(filename)
        data = img['data']
        # data=[np.asarray(image) for image in img['data']]
        ...
        results['img']=data
        # num_classes=3
        # one_hot_labels = torch.nn.functional.one_hot(img['label'], num_classes=num_classes)
        if self.with_label:
            results['gt_label']=img['label']
